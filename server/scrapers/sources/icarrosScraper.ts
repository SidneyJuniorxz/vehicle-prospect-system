...
/**
 * iCarros Scraper
 * Handles scraping of vehicle ads from iCarros with real HTML parsing
 */
export class iCarrosScraper extends BaseScraper {
  ...
  private parseAds(html: string, criteria: Record<string, any>): ScrapedVehicleAd[] {
    ...
    try {
      ...
      // iCarros item structure
      $("div.car-item, article.vehicle, div[data-testid='vehicle-card']").each(
        (_, element) => {
          try {
            ...
            // Extract title
            const title = $element.find("h2, .vehicle-title").text() || "";

            // Extract price
            const priceText = $element.find(".price, .vehicle-price").text() || "";
            ...
            console.error("[iCarros] Error parsing individual ad:", error);
          }
        }
      );
      ...
      console.error("[iCarros] Error parsing HTML:", error);
      return [];
    }
  }

  private extractIdFromUrl(url: string): string {
    // Extract ID from iCarros URL format: /veiculo/{id}
